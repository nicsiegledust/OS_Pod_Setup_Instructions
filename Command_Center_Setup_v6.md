# Command Center - Pod Setup Playbook v6
...
> **Snapshot date: 2026-06-08.** This document describes the system as of this date; live Pods evolve (see STATUS.md + README.md for current state).
...
4. **Wire ONE wake-up:** create the engine conversation ("Morning Run Engine") via pod_manager, schedule the weekday cron, record IDs in `how-i-work/schedule.json` + `logs/handoff-status.json` exactly as in Part 7 (the relay protocol applies unchanged).
...
**Upgrade path:** when the user says "upgrade my command center", re-run this file: keep all stored answers, resume the interview at Q8, and install the delta (Parts 2-7 per the new mode). The simple playbook is retired in favor of the full Morning Run; logs and the watchlist carry over.

---

## PART 2 - File System Seed

**Modes:** Standard and Full only (Simple uses the minimal layout in Part S). Standard skips the Periscope state files (`recommendations.csv`, `readiness-history.csv`, `power-coverage.json`) and `play-runner.md` / `play-card-spec.md`.
...
1. The operator answers the interview as proxy. Mark any answer they were unsure about, then confirm ALL uncertain answers via ONE Slack DM to the end user, confirmation-style ("Nic set your morning brief for 6 AM in #jane-daily - good?"), never a bare questionnaire.
...
*End of v6. Changelog: built 2026-06-08 from v5 (2026-06-07) plus first-external-user feedback. New in v6: self-triggering agent preamble (the file acts when dropped into a Pod), the plain-language contract and glossary (every interview question phrased as an outcome, subsystems explained before they are named), Simple / Standard / Full modes with the Part S simple install track and an additive upgrade path, role packs beyond sales (Appendix E), Part 11 "You're Live" orientation (post-setup message covering what happens next, what the system can see, and what it will never do alone), and setup-by-proxy for ops-led rollouts. The v5 GTM-specific edition remains in this repo as lineage.*
