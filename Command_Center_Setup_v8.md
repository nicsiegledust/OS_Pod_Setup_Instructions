...
> **Snapshot date: 2026-06-15.** This document describes the system as of this date; live Pods evolve (see STATUS.md + README.md for current state).
...
2. **Never name a subsystem in a question before explaining it in one plain sentence.** Internal names (Periscope, Cleanup Engine, engines, Play Cards, heartbeat) are fine INSIDE playbooks and specs; at the user-facing boundary they must be translated first.
...
| Cleanup Engine | A silent watchdog that only speaks if the system breaks (renamed from "Sentinel" in v8; same job) |
...
| Part 3 engines | Morning (simple variant) only | Morning + Nightly + Cleanup Engine | all four |
...
playbooks/           <- step-by-step instructions agents EXECUTE
  morning-run.md  nightly-review.md  weekly-prep.md  cleanup.md
...
**Modes:** Standard installs Morning Run + Nightly Review + Cleanup Engine (skip the Periscope steps inside them and the Weekly Prep engine). Full installs everything.

Write the engine playbooks from the templates in Appendix A, filling `{{PLACEHOLDERS}}` from the interview. The four engines:

### 🚒 Morning Engine (weekdays, rep-chosen time; canonical title "🚒 Morning Engine")
...
### 🚒 Evening Engine (weekdays 8 PM; canonical title "🚒 Evening Engine", was "Nightly Review")
...
### 🚒 Weekly Prep Engine (Sunday 7 PM)
...
### Cleanup Engine (weekdays 12 PM) - renamed from Sentinel in v8
The out-of-band dead-man switch. The other engines' watchdogs all live INSIDE the same engine family, so a relay failure could blind the whole system for days. The Cleanup Engine runs in its own conversation and does almost nothing on purpose:
...
Hard cap: 2 file reads + 1 status write per healthy fire. Never grow this playbook's scope; the Cleanup Engine's value is that it cannot fail in interesting ways. Weekend misfires exit silently.

### Engine Liveness Watchdogs (install in all engine playbooks)
The engines watch each other; the morning Slack post is the primary dead-man switch and the Cleanup Engine is the backstop:
...
## PART 3.5 - v8 update: thin Morning Engine + spawned daily worker (Full mode; Standard may adopt for context hygiene)

A weekday engine that runs all of Part 3's Morning Run steps in its OWN conversation accumulates ~6.5 weeks of context before its wake-up expires (Part 7), and context quality degrades well before that. **v8 splits the Morning Run in two:**

1. **🚒 Morning Engine (thin bootloader).** Stays context-light forever. On every weekday fire it: (a) reads `README.md`, `how-i-work/schedule.json`, and `system/handoff-status.json`; (b) self-reports liveness (`lastRun`, `fireCount`, status); (c) creates (or reuses, if already created today) a fresh conversation titled `Daily Prep - YYYY-MM-DD` via `pod_manager create_conversation`, with a kickoff message that says: "Execute `playbooks/daily-prep-run.md` top to bottom for YYYY-MM-DD. Read README.md first. You are the single writer for today's research, daily log, banner data, calendar events (Compass + Daily Research + Prep, Part 4.5), Pod tasks, Slack posts, weather image, audio briefing, and any Step 7.5 spawns. Do not run wakeup-relay.md; the 🚒 Morning Engine owns wake-up lifecycle."; (d) records the worker's conversation ID/URL in `handoff-status.json`; (e) runs the wake-up relay (Part 7) itself. **The 🚒 Morning Engine never does research, never writes the frame, and never posts to Slack.**

2. **Daily Prep worker (`playbooks/daily-prep-run.md`).** A fresh, non-recurring conversation every weekday. Executes ALL of Part 3's Morning Run steps (0 through 7.5) inside itself, including Step 2.6 below. It does NOT run the wake-up relay (it has no wake-up to renew); that stays with the thin 🚒 Morning Engine.

This is purely an internal restructuring: the rep-visible behavior (one Slack thread, one frame, one banner refresh) is unchanged. Simple mode does not need this split (its single playbook is small enough to stay in one long-lived conversation). Standard/Full SHOULD adopt it once the Morning Run playbook grows past a few hundred lines.

### Step 2.6 - Weekly meeting confirmation and gap-fill (v8, requires Part 3.6)
Each weekday, before regenerating the frame (Step 3), the daily worker re-confirms this week's already-prepped meetings against the LIVE calendar: drop cancelled ones, fully prep any meeting added or materially changed since the last full-week pass, and refresh the whole-week banner data (`calendar.events[]` + the weekly meeting summary for all 5 days, not just today).

---

## PART 3.6 - v8 update: full-week meeting prep + calendar QA guards

Earlier versions prepped only the next 1-2 meetings; the banner showed day-pills for the whole week but only today's meetings carried real prep data. v8 fixes this with three changes to `playbooks/meeting-prep.md`:

1. **Three explicit run modes.** **FULL-WEEK** (run by 🚒 Weekly Prep Engine on Sunday: prep EVERY external meeting Monday-Friday and write the entire week into the banner). **NEXT-DAY** (run by 🚒 Evening Engine: prep tomorrow's meetings only, skip Fridays since Sunday covers Monday). **CONFIRM + GAP-FILL** (run by the daily worker's Step 2.6: re-check the live calendar against existing prep, repair or create prep only for what changed).
2. **Calendar prep-event QA gate.** Every prepped meeting gets a `📋 [Prep Doc] <Company> (<ExternalLead>) / <YourCompany> (<YourPeople>)` calendar event (this title normalization is itself idempotent and never notifies guests). After writing prep events, re-read them: each must contain a QUICK GLANCE (call type, account, your-product footprint if you sell a product with telemetry, contacts, best hook, objective), known-contacts section, and (for higher-stakes call types: exec, demo, renewal, commercial, broad, or anything flagged risky) verified LinkedIn evidence for each external attendee. A prep event missing these is a QA failure for high-stakes calls and a logged gap for routine ones.
3. **Player LinkedIn / title normalization guard.** Attendee names are matched to LinkedIn profiles by name + company + role (never guessed); missing or ambiguous matches are recorded explicitly rather than silently dropped. The calendar event title itself is normalized to the `Company (Lead) / YourCompany (YourPeople)` format described above, recorded as `original_calendar_title` / `normalized_calendar_title` in the prep file's frontmatter.

Both 🚒 Evening Engine and 🚒 Weekly Prep Engine end their meeting-prep phase with a post-run QA sweep: counts of external meetings vs. prep files vs. prep calendar events vs. repaired descriptions vs. orphan warnings, logged to `system_notes`.
...
## PART 4.5 - v8 update: the Compass / Briefing split (replaces the single North Star event)

Earlier versions wrote one merged "North Star" calendar event (a ~60-minute cockpit mixing the rep's durable priorities, today's strategy, detailed research links, and a task board). v8 splits this into two short calendar events, both created/updated by the daily worker (Part 3.5), both on the rep's primary work calendar, both FREE/transparent with no attendees:

| Surface | Time (rep-local) | Title prefix | Holds |
|---|---:|---|---|
| **Compass** | 30 min, right after the morning run | `🧭 Compass` | Short strategy only: durable PRIORITIES (carried verbatim, never AI-edited), date-specific FOCUS, a short AI-written TODAY distillation (2-3 outcome sentences), NOT TODAY suppression lines, and OPEN LOOPS (durable scratchpad). At most 1-3 numeric `[1]`/`[2]` references to the Briefing in a bottom `Refs` block. No raw URLs, no task board, no research dump, no link list. One phone screen.
| **Daily Research + Prep ("the Briefing")** | 30 min, immediately after Compass | `🔬 Daily Research + Prep` | Everything that would bloat Compass: research findings and overnight signals, the task/action board (DUE / AT RISK, recommended actions, A/B/C choices), calendar work-block state, and links (daily conversation, frame, Slack posts, audio, weather animal, CRM records, prep docs). Idempotent: search-then-update per local date, never duplicate.

**Core principle: split intent from evidence.** Compass is what the rep WANTS to do and is thinking about; the Briefing is everything the system found and can act on. The rep edits Compass dash-lines directly (PRIORITIES, FOCUS, NOT TODAY, OPEN LOOPS) - the AI never rewrites, reorders, or deletes a rep-owned dash line. AI-written lines use a different bullet (e.g. `•`) so the two are visually distinct, and AI-owned status lines use a distinct prefix (e.g. `↳`) so a glance shows whether a line already has a linked task, an in-flight conversation, or a calendar work block.

**Optional command grammar** (Full mode, on Compass lines): a short prefix vocabulary lets the rep turn any Compass line into action without leaving the calendar - e.g. one symbol means "just tell me the status", another means "make this a task", another means "start working on this now", another means "block time on my calendar for this", and combinations of these. Exact symbols are a per-deployment choice; document whatever you pick in `specs/compass.md` and keep the legend in the Compass template itself so it is self-explanatory.

**Sync rules:** the daily worker reads today's Compass FIRST (it is the primary source of rep intent), then falls back to the durable mirrors (`my-context/priorities.json`, `my-context/scratchpad.md`), open Pod tasks, and yesterday's Compass. It builds the detailed Briefing first, then distills Compass from it. The Nightly Review reads Compass as the primary source of completed/changed intent (diffing PRIORITIES and OPEN LOOPS against the durable mirrors, newest edit wins) and reads the Briefing only as SECONDARY evidence (completed actions, links, follow-ups) - it must never sync PRIORITIES/FOCUS/OPEN LOOPS/NOT TODAY from the Briefing, and must never promote Briefing link-detail back into Compass.

A separate personal-namespace scratchpad event (different calendar, different title prefix) may exist for the rep's non-work thinking. It is NOT read by the work Morning Run unless the rep explicitly asks - keep work and personal intent surfaces privacy-separated by default.
...
## PART 5.8 - v8 update: Pod tasks as the single commitment ledger + task naming convention

Earlier versions kept tasks (a 72-hour action projection) and a separate "commitments" ledger (everything the rep owes, with owner/next-action/close-criteria) as two surfaces that drifted out of sync. **v8 merges them: Pod tasks ARE the commitment ledger.** A task counts as a commitment once it has an owner (default: the rep, unless genuinely someone else's), a next action, and close criteria. Any legacy commitments file becomes read-only context during migration and must not be expanded as a second ledger.

**Hard cap:** never carry more than ~10 open tasks for the rep; ideal range 5-10. If a run would exceed the cap, consolidate related work into ONE umbrella task with a checklist sub-document (never one task per sub-step), defer lower-priority items, or ask the rep to schedule/defer/kill before adding more. Compass shows only the top 2-3 must-win tasks even if the board holds up to the cap.

**Naming convention (canonical title format for every Pod task this system creates):**

```
P{0|1|2|3} · {explicit due date or "No date"} · {relative due date} · {Account/Customer if any}: {short action}
```

- **Priority** (`P0`-`P3`) comes first.
- **Explicit due date** comes second - a real calendar date (e.g. `Jun 15`) when known, or the literal `No date` when genuinely unscheduled.
- **Relative due date** comes third - `Today`, `Tomorrow`, `Overdue`, a short weekday label, or `Unscheduled`. Never bury this in free text; refresh it as time passes when carrying a task forward.
- If the task concerns a specific account/customer, that name comes next, followed by a colon, then a tight one-line action description.

Examples: `P1 · Jun 15 · Today · Acme: review ICP-leader emails and send the trial follow-up.` / `P1 · Jun 12 · Overdue · Globex: review deal setup and choose first stakeholder path.` / `P3 · No date · Unscheduled · Pod setup: spin up your own Pod from this file.`

**Every writer obeys this contract**: the daily worker (Part 3.5 Step 5), the Periscope play-runner (Part 5 dispatch), and any spawned workstream conversation. All three always assign to the rep unless genuinely someone else's; never leave a task unassigned (it falls out of "mine + open" views).

**Aging is enforced, not just documented**, by the 🚒 Evening Engine: a task important and older than 3 business days gets an age-escalation flag (surfaced in the frame and Slack); a task older than 5 business days triggers an explicit schedule/defer/kill question in the EOD note. Never auto-kill a stale task. A Periscope card being dismissed or skipped NEVER closes its linked task - only a real delivered outcome does. Standing rules ("always log X", "always route Y") are not tasks; write them into the owning playbook instead.

**Calendar-backed tasks (optional, v8):** every active task SHOULD reserve a Google/Outlook calendar work block by the next workday unless explicitly marked later/waiting/blocked. Minimum 15 minutes, default 30, 45-60 for deep work. The calendar event is a launcher (e.g. titled `🎯 Work Block · {task}`); Pod tasks remain the source of truth.
...
**Atomic write (v8):** write `dashboard_data.json` via a write-temp-then-rename pattern (write to a `.tmp` path, then atomically move it over the real path) so a crash or timeout mid-write can never leave the banner reading a truncated/corrupt JSON file. A small helper script in `playbooks/scripts/` (or equivalent) is enough; every writer of this file should call it.
...
1. Create the engine conversation via `pod_manager create_conversation` (titles: "🚒 Morning Engine", "🚒 Evening Engine", "🚒 Weekly Prep Engine", "🚒 Cleanup Engine" (the canonical v8 names; do not append month/date suffixes to these four titles)), agentName = the chosen runner. Kickoff message = "Each time you are invoked here, execute `playbooks/<engine>.md` top to bottom, then run `playbooks/wakeup-relay.md`." Keep the wake-up reason MINIMAL and defer to the playbook files; long reasons drift from the source of truth.
...
- fireCount >= 20 (v5: lowered from 26; an early relay keeps the successor conversation's CONTEXT fresh, not just the wake-up alive): create a successor conversation whose only job is to (1) schedule its own identical wake-up and (2) write `successor.confirmed: true` + its IDs into `handoff-status.json`. **v8: title the successor with the EXACT canonical engine title** (e.g. `🚒 Morning Engine`, not `🚒 Morning Engine - June 2026`) so the four-engine audit (Part 7.5) keeps matching by title. The remaining fires are the retry window; never assume one attempt succeeded.
...
## PART 7.5 - v8 update: the four-engine schedule health audit

In Full mode there should ALWAYS be exactly FOUR recurring engine wake-ups: 🚒 Morning Engine, 🚒 Evening Engine, 🚒 Weekly Prep Engine, 🚒 Cleanup Engine (Standard mode runs three, omitting Weekly Prep). Relay handoffs (previous section) are the most likely way this drifts: a successor gets created but never confirmed, or gets created with a slightly wrong title, and now there are 3 or 5 wake-ups instead of 4. v8 adds an explicit audit, run by the 🚒 Cleanup Engine on every weekday fire and again by the 🚒 Weekly Prep Engine on Sundays as a backstop (if the two disagree, trust the live wake-up list):

1. Read `how-i-work/schedule.json` -> `scheduledEngines` and `engineConversationTitles`, and `system/handoff-status.json` -> `engines`.
2. List active recurring wake-ups. Exclude spawned, non-recurring workers (e.g. `Daily Prep - YYYY-MM-DD`, `Deal Heartbeat - *`, `Deal Setup - *`, `Multithread - *`) - only the four canonical engines count.
3. **PASS** requires ALL of: exactly 4 (or 3 in Standard) recurring engine wake-ups active; their titles are exactly the canonical engine titles (no month/date suffixes); each engine's active wake-up ID matches `schedule.json` and its conversation ID matches `handoff-status.json`; no engine's `lastRun` is overdue for its cron by more than one full cycle.
4. On PASS: do nothing visible (silence is health). Record `{date, result: "pass", engineCount}` in `handoff-status.json.engines.cleanup.lastEngineAudit`.
5. On FAIL (wrong count, ID mismatch, stale/duplicate title, missing/expired wake-up, or overdue `lastRun`): post ONE consolidated alert to the ops Slack channel naming the expected count, the found count, and a per-engine status line (missing / stale title / ID mismatch / overdue / OK). **Alert only - never fix, reschedule, or cancel anything automatically.** Record the failure the same way.

This audit is intentionally read-only and cheap (one `list_wakeups` call plus two file reads); it exists so a silent relay failure surfaces within one business day instead of being discovered weeks later when a whole engine has quietly stopped running.
...
- [ ] `schedule.json` IDs match `handoff-status.json` IDs match the live wake-ups (probe-verified, `lastVerified` stamped), including the Cleanup Engine.
- [ ] (Full mode) the Part 7.5 four-engine audit passes: exactly 4 recurring engine wake-ups, canonical titles, IDs match `schedule.json` and `handoff-status.json`.
...
3. **Cleanup Engine test:** with `lastRefreshed` still set back, ask the Cleanup Engine conversation to dry-run its playbook and state what it would post; restore and confirm it would stay silent.
...
- **Week 1 hygiene:** confirm retired/legacy agents stopped posting; confirm retention cleanup ran; skim `handoff-status.json` once; verify the Cleanup Engine stayed silent on healthy days.
- **Weeks 4-5:** the first relay handoffs occur automatically around fireCount 20. The morning Slack post is the dead-man switch and the Cleanup Engine is the backstop: if the system ever goes silent, check `system/handoff-status.json` first.
...
4. The operator is recorded in STATUS.md as the maintainer. Watchdog/Cleanup Engine alerts and relay warnings go to the OPERATOR (their channel or DM), never to the end user; the end user only ever sees the brief and its threads.
...
3. **Wake-ups are mortal (32 fires).** The relay protocol plus watchdogs plus the Cleanup Engine is what keeps the system alive. Never hardcode wake-up IDs in playbooks.
...
```json
{
  ...
  "engines": {
    ...
    "cleanup":      { "conversationId": null, "wakeUpId": null, "cron": "{{CLEANUP_CRON}}", "playbook": "playbooks/cleanup.md",       "lastRun": null, "fireCount": 0, "status": "pending", "successor": { "created": false, "confirmed": false, "conversationId": null, "wakeUpId": null }, "handoffComplete": false, "lastVerified": null }
  }
}
```
...
```json
{
  ...
  "cleanupWakeup": { "cron": "{{CLEANUP_CRON}}", "playbook": "playbooks/cleanup.md", "wakeUpId": null, "conversationId": null, "title": "🚒 Cleanup Engine", "relaySuccessorAgent": "{{GENERAL_PURPOSE_AGENT}}" },
...
## --- playbooks/cleanup.md ---
Copy the Cleanup Engine section from Part 3 verbatim, with the channel ID inline and the hard cap ("2 file reads + 1 status write per healthy fire") stated as a guardrail.
